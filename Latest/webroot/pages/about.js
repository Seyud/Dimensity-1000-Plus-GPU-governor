/**
 * dimensity_hybrid_governor WebUI 关于页面模块
 * 显示模块信息和版本
 */
...
const AboutPage = {
    ...
    render() {
        ...
        <div class="about-container">
            <div class="about-header">
                ...
                <h2>dimensity_hybrid_governor WebUI</h2>
                <div class="version-badge">
                    <span>${I18n.translate('VERSION', '版本')} ${this.version}</span>
                </div>
                <p class="about-description">${I18n.translate('ABOUT_DESCRIPTION', 'dimensity_hybrid_governor模块管理界面')}</p>
            </div>
                ...
                <div class="about-section">
                    ...
                    <div class="developer-info">
                        <div class="developer-name">
                            <span>${this.moduleInfo.author || '酷安@瓦力喀'}</span>
                        </div>
    ...
    renderModuleInfo() {
        // 添加默认模块信息，以防加载失败
        if (Object.keys(this.moduleInfo).length === 0) {
            this.moduleInfo = {
        ...
        const infoItems = [
            { key: 'name', label: 'MODULE_NAME', icon: 'tag' },
            { key: 'version', label: 'MODULE_VERSION', icon: 'new_releases' },
            { key: 'versionCode', label: 'VERSION_DATE', icon: 'update' },
            { key: 'description', label: 'MODULE_DESCRIPTION', icon: 'description' },
            { key: 'id', label: 'MODULE_ID', icon: 'fingerprint' }
        ];
    ...
    async openModuleGitHubLink() {
        try {
            // 使用固定的GitHub链接
            const githubUrl = "https://github.com/Seyud";

            // 使用安卓浏览器打开链接
            await Core.execCommand(`am start -a android.intent.action.VIEW -d "${githubUrl}"`);
            console.log('已打开模块GitHub链接:', githubUrl);
        } catch (error) {
